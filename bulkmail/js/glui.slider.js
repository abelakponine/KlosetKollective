                ...
                $('.slider,.slider .rowspan').on('mousedown touchstart', function(event){ //plugin start
                    ...
                    var offset4 = (event.screenY || event.originalEvent.touches[0].screenY);
                        ...
                        $('html,.slider, .slider .rowspan').on('mouseup touchend', function(event){
                            ...
                            var a = $('.slider .rowspan').outerWidth(); //slider rowspan outerWidth
                            var b = $('.slider .grid').outerWidth();    //slider grid outerWidth
                            var c = Math.floor($('.slider').outerWidth()/$('.slider .grid').outerWidth()); //number of visible grid in slider
                            var d = Math.floor($('.slider .rowspan').outerWidth()/(b*(c))); //number of slides in slider rowspan

var e = Math.floor($('.slider .rowspan').outerWidth()/(b)); //number of grid in slider rowspan


var f = $('.slider .rowspan').outerWidth()-((b*c)*d); //number of grid in slider rowspan
...
var g = ((b*c)*d);
                            ...
                            //final offsetEnd calculation
                            if (newOffsetXOrig <= (((a-(b*c))*(d-1)+((a-(b*c))*d))*-1) && offsetDistX < 0){
                                $(obj.rowspan).css({'left':-((a-(b*c))*(d-1)+((a-(b*c))*d))+'px','transition-duration':'0.1s'});
                            }
                            
                            /*
                            if ($('.slider .grid').length < c){
                                $(obj.rowspan).css({'left':'0px','transition-duration':'0.1s'});
                            }
                            */
                            
                            if (newOffsetXOrig <= -g){
                                $(obj.rowspan).css({'left':(-g)+'px','transition-duration':'0.1s'});
                            }
